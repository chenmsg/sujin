    ...
    public class UploadController : Controller
    {
        ...
        public string UpImg(int cid=0,int UserId=0)
        {
            ...
            try
            {
                ...
                string path = "upload/users/"+DateTime.Now.ToString("yyyyMMdd")+"/";
                ...
                string file_ex = ".jpg";
                string url2 = Constant.CurrentApiHost + path;
                //文件名
                ...
                userImage.FileName = fileName + file_ex;
                userImage.Ip = Ip.GetClientIp() ;
                userImage.PlatForm = cid;
                userImage.State =0;
                userImage.Url = path+fileName+file_ex;
                userImage.UserId = UserId;
                int result = userImageDao.Insert(userImage);
                JObject data = new JObject();
                data["ImgUrl"] = aUrl;
                //data["filepath"] = dicPath + fileName + file_ex;
                data["ID"] = result;
                data["fileLength"] = fileLength;
                userEventDao.UserEventInit(cid, UserId, Ip.GetClientIp(), result>0?1:0, "Upload", "UpImg", $"{{ImgUrl:{aUrl},version:{TQuery.GetString("version")}}}");

                return ApiReturnStr.getApiData(result>0?0:-100, result > 0 ? "上传成功" :"上传失败", data);
            }
            catch (Exception ex)
            {
