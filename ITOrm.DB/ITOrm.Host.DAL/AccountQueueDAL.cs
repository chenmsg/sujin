...
namespace ITOrm.Host.DAL
{
    ...
    /// 备注接口
    /// </summary>
    public interface IAccountQueueDAL
    {
		#region ==========增删改数据
        /// <summary>
        /// 插入数据
        ...
        int Insert(AccountQueue entity);

        /// <summary>
        /// Update的对象，必须通过Single()获取重置属性后操作！传入实体修改，根据传入的实体主健修改，如果是new出来的实体，则要把Single()的对象赋给他才可更新
        /// </summary>
        /// <returns>返回,成功:true,失败:false</returns>
        bool Update(AccountQueue entity);

        /// <summary>
        /// Delete，根据实体对象删除
        /// </summary>
        /// <returns>返回,成功:true,失败:false</returns>
        bool Delete(AccountQueue entity);
        ...
        /// <returns>AccountQueue实体对象</returns>
        AccountQueue Single(string where, object param = null);
        ...
        List<AccountQueue> GetQuery(string where, object param = null, string orderBy = null);
        ...
        /// <param name="orderBy">排序语句[例子：order by id desc ]</param>
        /// <returns>List集合</returns>
        List<AccountQueue> GetQuery(int top, string where, object param = null, string orderBy = null);
        ...
        /// <returns></returns>
        List<AccountQueue> GetPaged(int pageSize, int pageIndex, out int totalCount, string where, object param = null, string orderBy = null);
    ...
    /// 备注
    /// </summary>
    public class AccountQueueDAL : BaseEntityAction<AccountQueue>, IAccountQueueDAL
    {
	    #region ==========增删改数据
        ...
        /// </summary>
        /// <returns>返回,成功:true,失败:false</returns>
        public new  int Insert(AccountQueue entity)
        {
        ...
        /// </summary>
        /// <returns>返回,成功:true,失败:false</returns>
        public new bool Update(AccountQueue entity)
        {
			if (entity == null) throw new ArgumentNullException("obj");
        ...
        /// </summary>
        /// <returns>返回,成功:true,失败:false</returns>
        public new  bool Delete(AccountQueue entity)
        {
            if (entity == null) throw new ArgumentNullException("obj");
        ...
        public AccountQueue Single(int id)
        {
            ...
            var result = Memcache.Get(key) as AccountQueue;
            if (result == null)
            {
        ...
        {
			            var result = base.GetQuery(string.Format("SELECT ID FROM AccountQueue WITH(NOLOCK) WHERE {0} {1}", where, orderBy), param);
        ...
        {
			            var result = base.GetQuery(string.Format("SELECT top {2} ID FROM AccountQueue WITH(NOLOCK) WHERE {0} {1} ", where, orderBy, top), param);
			return MappingCacheEntity(result);
			        }
        ...
        {
            return base.Count("SELECT COUNT(0) FROM AccountQueue WITH(NOLOCK) WHERE " + where, param);
        }
        ...
        public List<AccountQueue> GetPaged(int pageSize, int pageIndex, out int totalCount, string where, object param = null, string orderBy = null)
        {
			...
			string field="ID";
            string sql = string.Format("SELECT {0} FROM AccountQueue ", field);
            if (topNum <= 1)
            {
				sql = string.Format("SELECT {0} FROM (SELECT ROW_NUMBER() OVER({1}) as 'RowNumber', {0} from AccountQueue WITH(NOLOCK) {3} ) as temp WHERE (RowNumber between 0 and {2} ) ", field, orderByNow, pageSize, (!string.IsNullOrEmpty(where) ? " WHERE " + where : ""));
            }
            else
            {
                sql = string.Format("SELECT {0} FROM (SELECT ROW_NUMBER() OVER({1}) as 'RowNumber', {0} from AccountQueue WITH(NOLOCK) {4} ) as temp WHERE (RowNumber between {2} and {3} ) ", field, orderByNow, topNum, (topNum + pageSize-1), (!string.IsNullOrEmpty(where) ? " WHERE " + where : ""));
            }
            try
            {
        ...
        {
            return Memcache.EncodeKey("SujinDB", "AccountQueue", paramName, paramValue);
        }
        ...
        /// <returns></returns>        
        private List<AccountQueue> MappingCacheEntity(List<int> result)
        {
            //exists result is null(empty)
            if (result != null && result.Count > 0)
            {
                String key = string.Empty;
                AccountQueue cache = null;
                List<AccountQueue> result_cache = new List<AccountQueue>(result.Count);

                foreach (var item in result)
                {
                    key = EncodeKey("ID", item);
                    cache = Memcache.Get(key) as AccountQueue;
                    if (cache == null)
                    {
                        cache = Single(item);
