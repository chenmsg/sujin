...
namespace ITOrm.Host.DAL
{
    ...
    public interface IKeyValueDAL
    {
        ...
        /// <returns>KeyValue实体对象</returns>
        KeyValue Single(string where, object param = null);
        ...
        List<KeyValue> GetQuery(string where, object param = null, string orderBy = null);
        ...
        /// <param name="orderBy">排序语句[例子：order by id desc ]</param>
        /// <returns>List集合</returns>
        List<KeyValue> GetQuery(int top, string where, object param = null, string orderBy = null);
        ...
        /// <returns></returns>
        List<KeyValue> GetPaged(int pageSize, int pageIndex, out int totalCount, string where, object param = null, string orderBy = null);
    ...
    /// 备注
    /// </summary>
    public class KeyValueDAL : BaseEntityAction<KeyValue>, IKeyValueDAL
    {
	    #region ==========增删改数据
        ...
        /// </summary>
        /// <returns>返回,成功:true,失败:false</returns>
        public new  int Insert(KeyValue entity)
        {
        ...
        /// </summary>
        /// <returns>返回,成功:true,失败:false</returns>
        public new bool Update(KeyValue entity)
        {
			if (entity == null) throw new ArgumentNullException("obj");
        ...
        /// </summary>
        /// <returns>返回,成功:true,失败:false</returns>
        public new  bool Delete(KeyValue entity)
        {
            if (entity == null) throw new ArgumentNullException("obj");
        ...
        public KeyValue Single(int id)
        {
            ...
            var result = Memcache.Get(key) as KeyValue;
            if (result == null)
            {
        ...
        {
			            var result = base.GetQuery(string.Format("SELECT ID FROM KeyValue WITH(NOLOCK) WHERE {0} {1}", where, orderBy), param);
        ...
        {
			            var result = base.GetQuery(string.Format("SELECT top {2} ID FROM KeyValue WITH(NOLOCK) WHERE {0} {1} ", where, orderBy, top), param);
			return MappingCacheEntity(result);
			        }
        ...
        {
            return base.Count("SELECT COUNT(0) FROM KeyValue WITH(NOLOCK) WHERE " + where, param);
        }
        ...
        public List<KeyValue> GetPaged(int pageSize, int pageIndex, out int totalCount, string where, object param = null, string orderBy = null)
        {
			...
			string field="ID";
            string sql = string.Format("SELECT {0} FROM KeyValue ", field);
            if (topNum <= 1)
            {
				sql = string.Format("SELECT {0} FROM (SELECT ROW_NUMBER() OVER({1}) as 'RowNumber', {0} from KeyValue WITH(NOLOCK) {3} ) as temp WHERE (RowNumber between 0 and {2} ) ", field, orderByNow, pageSize, (!string.IsNullOrEmpty(where) ? " WHERE " + where : ""));
            }
            else
            {
                sql = string.Format("SELECT {0} FROM (SELECT ROW_NUMBER() OVER({1}) as 'RowNumber', {0} from KeyValue WITH(NOLOCK) {4} ) as temp WHERE (RowNumber between {2} and {3} ) ", field, orderByNow, topNum, (topNum + pageSize-1), (!string.IsNullOrEmpty(where) ? " WHERE " + where : ""));
            }
            try
            {
        ...
        {
            return Memcache.EncodeKey("SujinDB", "KeyValue", paramName, paramValue);
        }
        ...
        /// <returns></returns>        
        private List<KeyValue> MappingCacheEntity(List<int> result)
        {
            ...
            if (result != null && result.Count > 0)
            {
                ...
                KeyValue cache = null;
                List<KeyValue> result_cache = new List<KeyValue>(result.Count);

                foreach (var item in result)
                {
                    key = EncodeKey("ID", item);
                    cache = Memcache.Get(key) as KeyValue;
                    if (cache == null)
                    {
                        cache = Single(item);
