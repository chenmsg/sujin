...
namespace ITOrm.Host.DAL
{
    ...
    /// 备注接口
    /// </summary>
    public interface IUserEventRecordDAL
    {
		#region ==========增删改数据
        /// <summary>
        /// 插入数据
        ...
        int Insert(UserEventRecord entity);

        /// <summary>
        /// Update的对象，必须通过Single()获取重置属性后操作！传入实体修改，根据传入的实体主健修改，如果是new出来的实体，则要把Single()的对象赋给他才可更新
        /// </summary>
        /// <returns>返回,成功:true,失败:false</returns>
        bool Update(UserEventRecord entity);

        /// <summary>
        /// Delete，根据实体对象删除
        /// </summary>
        /// <returns>返回,成功:true,失败:false</returns>
        bool Delete(UserEventRecord entity);
        ...
        /// <returns>UserEventRecord实体对象</returns>
        UserEventRecord Single(string where, object param = null);
        ...
        List<UserEventRecord> GetQuery(string where, object param = null, string orderBy = null);
        ...
        /// <param name="orderBy">排序语句[例子：order by id desc ]</param>
        /// <returns>List集合</returns>
        List<UserEventRecord> GetQuery(int top, string where, object param = null, string orderBy = null);
        ...
        /// <returns></returns>
        List<UserEventRecord> GetPaged(int pageSize, int pageIndex, out int totalCount, string where, object param = null, string orderBy = null);
    ...
    /// 备注
    /// </summary>
    public class UserEventRecordDAL : BaseEntityAction<UserEventRecord>, IUserEventRecordDAL
    {
	    #region ==========增删改数据
        ...
        /// </summary>
        /// <returns>返回,成功:true,失败:false</returns>
        public new  int Insert(UserEventRecord entity)
        {
        ...
        /// </summary>
        /// <returns>返回,成功:true,失败:false</returns>
        public new bool Update(UserEventRecord entity)
        {
			if (entity == null) throw new ArgumentNullException("obj");
        ...
        /// </summary>
        /// <returns>返回,成功:true,失败:false</returns>
        public new  bool Delete(UserEventRecord entity)
        {
            if (entity == null) throw new ArgumentNullException("obj");
        ...
        public UserEventRecord Single(int id)
        {
            ...
            var result = Memcache.Get(key) as UserEventRecord;
            if (result == null)
            {
        ...
        {
			            var result = base.GetQuery(string.Format("SELECT ID FROM UserEventRecord WITH(NOLOCK) WHERE {0} {1}", where, orderBy), param);
        ...
        {
			            var result = base.GetQuery(string.Format("SELECT top {2} ID FROM UserEventRecord WITH(NOLOCK) WHERE {0} {1} ", where, orderBy, top), param);
			return MappingCacheEntity(result);
			        }
        ...
        {
            return base.Count("SELECT COUNT(0) FROM UserEventRecord WITH(NOLOCK) WHERE " + where, param);
        }
        ...
        public List<UserEventRecord> GetPaged(int pageSize, int pageIndex, out int totalCount, string where, object param = null, string orderBy = null)
        {
			...
			string field="ID";
            string sql = string.Format("SELECT {0} FROM UserEventRecord ", field);
            if (topNum <= 1)
            {
				sql = string.Format("SELECT {0} FROM (SELECT ROW_NUMBER() OVER({1}) as 'RowNumber', {0} from UserEventRecord WITH(NOLOCK) {3} ) as temp WHERE (RowNumber between 0 and {2} ) ", field, orderByNow, pageSize, (!string.IsNullOrEmpty(where) ? " WHERE " + where : ""));
            }
            else
            {
                sql = string.Format("SELECT {0} FROM (SELECT ROW_NUMBER() OVER({1}) as 'RowNumber', {0} from UserEventRecord WITH(NOLOCK) {4} ) as temp WHERE (RowNumber between {2} and {3} ) ", field, orderByNow, topNum, (topNum + pageSize-1), (!string.IsNullOrEmpty(where) ? " WHERE " + where : ""));
            }
            try
            {
        ...
        /// <returns></returns>        
        private List<UserEventRecord> MappingCacheEntity(List<int> result)
        {
            //exists result is null(empty)
            if (result != null && result.Count > 0)
            {
                String key = string.Empty;
                UserEventRecord cache = null;
                List<UserEventRecord> result_cache = new List<UserEventRecord>(result.Count);

                foreach (var item in result)
                {
                    key = EncodeKey("ID", item);
                    cache = Memcache.Get(key) as UserEventRecord;
                    if (cache == null)
                    {
                        cache = Single(item);
