...
namespace ITOrm.Host.DAL
{
    ...
    public interface IBankDAL
    {
        ...
        /// <returns>Bank实体对象</returns>
        Bank Single(string where, object param = null);
        ...
        List<Bank> GetQuery(string where, object param = null, string orderBy = null);
        ...
        /// <param name="orderBy">排序语句[例子：order by id desc ]</param>
        /// <returns>List集合</returns>
        List<Bank> GetQuery(int top, string where, object param = null, string orderBy = null);
        ...
        /// <returns></returns>
        List<Bank> GetPaged(int pageSize, int pageIndex, out int totalCount, string where, object param = null, string orderBy = null);
    ...
    /// 备注
    /// </summary>
    public class BankDAL : BaseEntityAction<Bank>, IBankDAL
    {
	    #region ==========增删改数据
        ...
        /// </summary>
        /// <returns>返回,成功:true,失败:false</returns>
        public new  int Insert(Bank entity)
        {
        ...
        /// </summary>
        /// <returns>返回,成功:true,失败:false</returns>
        public new bool Update(Bank entity)
        {
			if (entity == null) throw new ArgumentNullException("obj");
        ...
        /// </summary>
        /// <returns>返回,成功:true,失败:false</returns>
        public new  bool Delete(Bank entity)
        {
            if (entity == null) throw new ArgumentNullException("obj");
        ...
        /// <param name="param">参数化对象</param>
        /// <returns>Bank实体对象</returns>
        public   Bank Single(string where, object param = null)
        {
        ...
        /// <param name="orderBy">排序语句[例子：order by id desc ]</param>
        /// <returns>List集合</returns>
        public List<Bank> GetQuery(string where, object param = null, string orderBy = null)
        {
			            var result = base.GetQuery(string.Format("SELECT ID FROM Bank WITH(NOLOCK) WHERE {0} {1}", where, orderBy), param);
        ...
        /// <param name="orderBy">排序语句[例子：order by id desc ]</param>
        /// <returns>List集合</returns>
        public List<Bank> GetQuery(int top, string where, object param = null, string orderBy = null)
        {
			            var result = base.GetQuery(string.Format("SELECT top {2} ID FROM Bank WITH(NOLOCK) WHERE {0} {1} ", where, orderBy, top), param);
			return MappingCacheEntity(result);
			        }
        ...
        {
            return base.Count("SELECT COUNT(0) FROM Bank WITH(NOLOCK) WHERE " + where, param);
        }
        ...
        /// <param name="orderBy">排序</param>
        /// <returns></returns>
        public List<Bank> GetPaged(int pageSize, int pageIndex, out int totalCount, string where, object param = null, string orderBy = null)
        {
			...
			string field="ID";
            string sql = string.Format("SELECT {0} FROM Bank ", field);
            if (topNum <= 1)
            {
				sql = string.Format("SELECT {0} FROM (SELECT ROW_NUMBER() OVER({1}) as 'RowNumber', {0} from Bank WITH(NOLOCK) {3} ) as temp WHERE (RowNumber between 0 and {2} ) ", field, orderByNow, pageSize, (!string.IsNullOrEmpty(where) ? " WHERE " + where : ""));
            }
            else
            {
                sql = string.Format("SELECT {0} FROM (SELECT ROW_NUMBER() OVER({1}) as 'RowNumber', {0} from Bank WITH(NOLOCK) {4} ) as temp WHERE (RowNumber between {2} and {3} ) ", field, orderByNow, topNum, (topNum + pageSize-1), (!string.IsNullOrEmpty(where) ? " WHERE " + where : ""));
            }
            try
            {
