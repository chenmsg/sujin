...
namespace ITOrm.EF.Models.Helper
{
    ...
    /// 数据访问助手类
    /// </summary>
    public partial class MssqlDbHelper
    {
        public MssqlDbHelper()
        {
        }

        public MssqlDbHelper(string Dbconnection)
        {
            if (!(string.IsNullOrEmpty(Dbconnection)))
            {
                m_connectionstring = Dbconnection;
                if (m_factory == null)
                {
                    m_factory.CreateConnection().ConnectionString = m_connectionstring;
                }
            }
        }
        ...
        /// DbProviderFactory实例
        /// </summary>
        private DbProviderFactory m_factory = new MssqlProvider().Instance();
        ...
        /// IDbProvider接口
        /// </summary>
        public IMssqlProvider Provider = new MssqlProvider();
        ...
        public int ExecuteNonQuery(out int id, DbConnection connection, CommandType commandType, string commandText, params DbParameter[] commandParameters)
        {
            ...
            id = Util.ObjectToInt(cmd.ExecuteScalar());
        ...
        /// <param name="commandText">存储过程名或都SQL命令文本</param>
        /// <returns></returns>
        public string GetRealConnectionString(string commandText, CommandType commandType)
        {
            //当在线表被布署到别的数据库时
