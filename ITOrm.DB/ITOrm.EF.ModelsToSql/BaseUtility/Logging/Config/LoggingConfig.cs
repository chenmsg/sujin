...
namespace ITOrm.EF.Models.Logging
{
	/// <summary>
	/// 日志组件配置
	/// </summary>
    public class LoggingConfig
	{
        ...
        {
            XmlNode root = (XmlNode)ConfigurationManager.GetSection("ClumpConfig/logging");
            if (root == null)
            {
                return;
            }
        ...
        private void ParseFormatters(XmlNodeList nodes)
        {
            foreach (XmlNode node in nodes)
            {
                ...
                {
                    Name = ConfigHelper.GetAttribute(node, "name", true),
                    Format = ConfigHelper.GetAttribute(node, "format", false)
                };

                string typeName = ConfigHelper.GetAttribute(node, "type", true);
                if (!typeName.Contains("."))
                    typeName = "ITOrm.EF.Models.Logging." + typeName;
        ...
        private void ParseLoggers(XmlNodeList nodes)
        {
            foreach (XmlNode node in nodes)
            {
                ...
                var config = new LoggerConfig
                {
                    ...
                    Namespace = ConfigHelper.GetAttribute(node, "namespace", "*"),//若namespace属性没配置，则默认为“*”，表示该Logger可以在任意地方写日志
                    Name = ConfigHelper.GetAttribute(node, "name", true)
                };

                string[] names = ConfigHelper.GetAttribute(node, "targets", true).Split(',');
                foreach (string name in names)
                    config.TargetNames.Add(name.Trim());

                try
                {
                    string minLevel = ConfigHelper.GetAttribute(node, "minLevel", false);
        ...
        private void ParseTargets(XmlNodeList nodes)
        {
            foreach (XmlNode node in nodes)
            {
                ...
                config.Name = ConfigHelper.GetAttribute(node, "name", true);
                config.ChildConfig = node.ChildNodes;

                config.FormatterName = ConfigHelper.GetAttribute(node, "formatter", false);
                if (string.IsNullOrEmpty(config.FormatterName))
                    config.FormatterName = "DefaultFormatter";

                string typeName = ConfigHelper.GetAttribute(node, "type", true);
                if (!typeName.Contains("."))
                    typeName = "ITOrm.EF.Models.Logging.Targets." + typeName;
