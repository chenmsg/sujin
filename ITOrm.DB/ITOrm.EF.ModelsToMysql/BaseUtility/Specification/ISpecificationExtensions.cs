...
namespace ITOrm.Ms.Models.Specification
{
    public static class ISpecificationExtensions
    {
        ...
        public static ISpecification<T> Like<T>(this ISpecification<T> q, Expression<Func<T, string>> property, string value)
        {
            ...
            {
                var expression = Expression.Lambda<Func<T, bool>>(Expression.Call(property.Body, typeof(string).GetMethod("Contains"), Expression.Constant(value)), property.Parameters);
        ...
        /// <param name="value">查询值</param>
        /// <returns></returns>
        public static ISpecification<T> OrLike<T>(this ISpecification<T> q, Expression<Func<T, string>> property, string value)
        {
            value = value.Trim();
            if (!string.IsNullOrEmpty(value))
            {
                var expression = Expression.Lambda<Func<T, bool>>(Expression.Call(property.Body, typeof(string).GetMethod("Contains"), Expression.Constant(value)), property.Parameters);
                q.Predicate = q.Predicate.Or(expression);
            }
            return q;
        }
        ...
        /// <param name="value">查询值</param>
        /// <returns></returns>
        public static ISpecification<T> OrEquals<T, P>(this ISpecification<T> q, Expression<Func<T, P>> property, P value)
        {
            var parameter = property.GetParameters();
            var constant = Expression.Constant(value);
            Type type = typeof(P);
            ...
            var methodExp = Expression.Equal(nonNullProperty, constant);
            Expression<Func<T, bool>> lambda = Expression.Lambda<Func<T, bool>>(methodExp, parameter);
            q.Predicate = q.Predicate.Or(lambda);
            return q;
        }
