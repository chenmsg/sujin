...
namespace ITOrm.Core.Helper
{
    ...
    public static class Util
    {
        ...
        {
            string randomChars = "QWERTYUIOPASDFGHJKLZXCVBNM012346789";
        ...
        public static List<double> BubblingSort(List<double> arrSort)
        {
            double temp;
        ...
        public static string RemoveAllHtml(string content)
        {
            ...
            content = content.Replace(" ", "").Replace("\n", "").Replace("\r", "").Replace("\x20", "").Replace("\u3000", "").Replace("\u0020", "");
            return content;
        }
        ...
        /// <param name="totalPage">数据总条数</param>
        /// <param name="pageIndex">当前页码</param>
        /// <param name="pageName">分页参数名</param>
        /// <param name="pageSize">每页条数</param>
        /// <param name="pageFenye">当前页的页码数</param>
        /// <param name="style">样式</param>
        /// <returns></returns>
        public static string Pager(int totalPage, int pageIndex, string pageName, int pageSize, int pageFenye)
        {
            string result = String.Empty;
            int pageNum = 0;//总页数
            pageNum = totalPage / pageSize;
            ...
            else
            {
                ...
                int numtr = 1;
                int pnow = 1;//当前是第几个pageFenye页
                string _name = String.Empty;
                sbuilder.Append(string.Format("<a href='/{0}/' >首页</a>", pageName));
                pnow = pageIndex / pageFenye;//当前是第几个10页
                if (pageIndex % pageFenye == 0)
                {
                    pnow = pnow - 1;
                }

                if (pageIndex > pageFenye && pageIndex == pnow * pageFenye + 1)
                {
                    numtr = pnow * pageFenye;
                }

                if (pageIndex > pageFenye && pageIndex % pageFenye == 0)
                {
                    numtr = pageIndex - pageFenye;// numtr = i - 1;
                }
                else if (pageIndex > pageFenye && pageIndex % pageFenye > 0)//&& i < 2 * pageFenye_duanluo
                {
                    numtr = pageFenye * pnow;
                }

                for (int j = numtr; j <= numtr + pageFenye + (numtr < pageFenye ? 0 : 1); j++)
                {
                    if (j > pageNum) { break; }
                    ...
                    if (j == 1)
                    {
                        ...
                        _name = string.Format("/{0}/", pageName);//网页名称
                    }
                    else
                    {
                        _name = string.Format("/{0}/page-{1}.html", pageName, j);//网页名称
                    }

                    if (numtr >= 1 && j == numtr + pageFenye + (numtr < pageFenye ? 0 : 1))
                    {
                        sbuilder.Append(string.Format("<a href='{0}' >...</a>", _name));
                    }
                    else if (numtr > 1 && j == numtr)
                    {
                        sbuilder.Append(string.Format("<a href='{0}' >...</a>", _name));
                    }
                    else
                    {
                        if (pageIndex == j)
                        {
                            sbuilder.Append(string.Format("<span>{0}</span>", j));
                        }
                        else
                        {
                            sbuilder.Append(string.Format("<a href='{0}' >{1}</a>", _name, j));
                        }
                    }
                }
                sbuilder.Append(string.Format("<a href='/{0}/page-{1}.html' >尾页</a>", pageName, pageNum));
